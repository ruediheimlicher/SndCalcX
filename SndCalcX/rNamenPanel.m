      ...
      for (i=0;i<[tempNamenViewArray count];i++)
      {
         ...
         if ([tempZeilenstring length])
         {
            ...
            if ([tempNamenArray count]>1)
            {
               ...
               VornamenString = [VornamenString stringByReplacingCharactersInRange:NSMakeRange(0,1)
                                                                  withString:[[VornamenString substringToIndex:1] capitalizedString]];

               if ([VornamenString doesMatchRegStringExp:nameregex])
               {
                  NSLog(@"Vorname %@ ist OK",VornamenString);
               }
               else
               {
                  fehler++;
                  NSLog(@"Vorname %@ ist nicht OK",VornamenString);
               }

               NamenString=[self stringSauberVon:[tempNamenArray objectAtIndex:[tempNamenArray count]-1]];
               
               NamenString = [NamenString stringByReplacingCharactersInRange:NSMakeRange(0,1)
                                                                         withString:[[NamenString substringToIndex:1] capitalizedString]];

               if ([NamenString doesMatchRegStringExp:nameregex])
               {
                  NSLog(@"NamenString %@ ist OK",NamenString);
               }
               else
               {
                  fehler++;
                  NSLog(@"NamenString %@ ist nicht OK",NamenString);
               }
               ...
               if (fehler) // falsche eingabe
               {
                  NSAlert* EingabeWarnung=[[NSAlert alloc]init];
            ...
            {
               VornamenString=[[self stringSauberVon:[tempNamenArray objectAtIndex:0]]uppercaseString];
...
{
   return (long)[NamenDicArray count];
}
