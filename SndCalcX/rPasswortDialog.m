...
{
   //if ((self = [super init]))
   self = [super initWithWindowNibName:@"SCPasswortDialog"];
...
- (IBAction)reportChange:(id)sender
{
   ...
   if ([[neuesPW2Feld stringValue]isEqualToString:[neuesPW1Feld stringValue]])//Passwort OK
   {
      ...
      //NSLog(@"reportChange	neuerPasswortDic: %@",[neuerPasswortDic description]);
      ...
      //NSLog(@"reportChange	NotificationDic: %@",[NotificationDic description]);
      NSNotificationCenter* nc=[NSNotificationCenter defaultCenter];
   ...
   else
   {
      ...
      [Warnung setMessageText:NSLocalizedString(@"Input Error:",@"Eingabefehler:")];
      NSString* s=NSLocalizedString(@"The second input does not match the first", @"Die zweite Eingabe stimmt nicht mit der ersten überein.");
