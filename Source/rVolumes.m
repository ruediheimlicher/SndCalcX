...
- (void) awakeFromNib
{
	...
	[AuswahlenKnopf setToolTip:NSLocalizedString(@"Choose the klicked user.",@"Den angeklickten Benutzer auswŠhlen.")];
	[NetzwerkKnopf setToolTip:NSLocalizedString(@"Open a panel to connect to a network user.",@"Ã–ffnet ein Dialogfeld, um die Verbindung zu einen Benutzer im Netzwerk einzurichten.")];
...
- (void) setUserArray:(NSArray*) dieUser
{
	...
	if ([dieUser count])
	{
		...
		while (einObjekt=[enumerator nextObject])
		{
			...
			BOOL userSndCalcDatenOK=NO;//Hat es einen Datenordner fŸr User?
			...
			BOOL volumeSndCalcDatenOK=NO;//Hat es einen Datenordner fŸr Volume?
			...
			/*
			 BOOL homeSndCalcDatenAngelegt=NO;//Hat es Daten im Datenordner fŸr home?
...
- (NSString*)chooseNetworkSndCalcDatenPfad
{
	...
	{
		tempSndCalcDatenPfad=[LeseboxDialog filename]; //gewÃ¤hltes "home" 
		...
		else
		{
			...
			[Warnung setMessageText:NSLocalizedString(@"This is not a valable path for the application",@"Kein gÃ¼ltiger Pfad")];
