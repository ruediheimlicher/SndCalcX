...
{
/*
Gibt den Pfad der SndCalcDaten auf home zurÃ¼ck
*/
...
{
	/*
	 PrŸft home und die eingeloggten Benutzer, ob ein ordner SndCalcDaten vorhanden ist. 
	 Gibt einen Array mit Dics zurŸck, 
	 die in den Userarray des Startfensters eingesetzt werden
	 Die Dics enthalten den Pfad und eine Anzeige fŸr den Ordner SndCalcDaten
	 */
	...
	NSMutableArray * UserMitSndCalcDatenArray=[NSMutableArray arrayWithCapacity:0];//Ordner fŸr die Dics
	NSMutableDictionary* HomeDic=[[NSMutableDictionary alloc]initWithCapacity:0];//Dic fuer Volume
	
	//Dic fŸr Home einrichten
...
{
	/*	
	Der returnDic enthŠlt die Daten unter dem Key "userplist"!!	
	*/
			...
			while (einName=[NamenEnum nextObject])
			{
				...
				if (tempUserPListDic && [tempUserPListDic objectForKey:@"ergebnisdicarray"])
				{
					...
					if ([tempInvalidErgebnisTestNamenArray count])//Es hat ungueltige Ergebnisse
					{
						...
						NSString* MessageString=NSLocalizedString(@"Delete invalid results",@"UngŸltige Ergebnisse lšschen");
						[Warnung setMessageText:[NSString stringWithFormat:@"%@",MessageString]];
						
						NSString* s1=NSLocalizedString(@" Results will be definetly deleted.",@" Ergebnisse werden endgŸltig gelšscht.");
						NSString* s2=@"";
...
{
/*
Gibt die Volumes im Ordner 'Network' zurÃ¼ck
*/
	...
	NSString* NetzPfad=@"//Network";
		...
		if ([NetzobjekteArray count])
		{
			...
			NSMutableDictionary* tempNetworkDic=[[NSMutableDictionary alloc]initWithCapacity:0];//Dic fÃ¼r Volume
