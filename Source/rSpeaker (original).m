		...
		if (IDausPlist)
		{
			...
			if ([Filemanager fileExistsAtPath:tempAIFFPfad])
         {
            ...
            [tempMovieDic setObject:tempAIFFPfad forKey:@"zahlpfad"];
				...
				[tempZahlenArray addObject:tempMovieDic];
			}
         else
         {
...
//	NSArray* sortArray=[tempZahlenArray sortedArrayUsingDescriptors:[NSArray arrayWithObjects:desc,nil]];
	...
	ZahlenArray=[NSArray arrayWithArray:[tempZahlenArray sortedArrayUsingDescriptors:[NSArray arrayWithObjects:desc,nil]]];
...
//							NSLog(@"home:		eineZahl: nur 1 Element",eineQuittung);
   ...
   NSLog(@"readQuittungen tempZahlenArray: %@",[tempZahlenArray description]);

	NSSortDescriptor* desc=[[NSSortDescriptor alloc] initWithKey:@"ID" ascending: YES];
	//NSArray* sortArray=[tempZahlenArray sortedArrayUsingDescriptors:[NSArray arrayWithObjects:desc,nil]];
	QuittungZahlenArray=[[tempZahlenArray copy] sortedArrayUsingDescriptors:[NSArray arrayWithObjects:desc,nil]];
	//NSLog(@"");
...
-(QTTime)setQTKitQuittungVon:(int)dieQuittung mitOffset:(QTTime)derOffset
{
	...
	NSArray* IDArray=[QuittungZahlenArray valueForKey:@"ID"];
   ...
   QuittungenQTKitMovie= [[QuittungZahlenArray objectAtIndex:QuittungIndex]objectForKey:@"movie"];
...
-(QTTime)setOpQTKitTrackVon:(int)dieOperation mitOffset:(QTTime)derOffset
{
	...
	NSArray* IDArray=[ZahlenArray valueForKey:@"ID"];
	//NSLog(@"IDArray: %@",[IDArray description]);
	int GermanOffset=20000;
	int IDOffset=GermanOffset;
   ...
   QTMovie* OperationQTKitMovie=[[ZahlenArray objectAtIndex:OperationIndex]objectForKey:@"movie"];
...
//		goto bail;
   ...
   QTMovie* HundertQTKitMovie=[[ZahlenArray objectAtIndex:HundertIndex]objectForKey:@"movie"];
   ...
   QTMovie* TausendQTKitMovie=[[ZahlenArray objectAtIndex:TausendIndex]objectForKey:@"movie"];
   ...
   if (Tausender>0)			//Es hat Tausender
	{
      ...
      TausenderQTKitMovie=[[ZahlenArray objectAtIndex:TausenderIndex]objectForKey:@"movie"];
   ...
   QTMovie* HunderterQTKitMovie;
	...
	{	
		...
		if ((Hunderter>1)||((Hunderter==1)&&(Tausender>0)))//Mehr als ein Hunderter oder mit Tausendern
		{
         ...
         HunderterQTKitMovie = [[ZahlenArray objectAtIndex:HunderterIndex]objectForKey:@"movie"];
...
//   Movie ZehnerQTMovie=nil;
   ...
   QTMovie* UndQTKitMovie;
	...
	if (Zehner>0)			// Es hat Zehner
	{
		if (Zehner>1)		// Ab 20
		{
         if (Einer>0)
         {
            ...
            // QTMovie fuer Einer
            EinerQTKitMovie=[[ZahlenArray objectAtIndex:EinerIndex]objectForKey:@"movie"];
            ...
            // QTMovie fuer "und"
            UndQTKitMovie=[[ZahlenArray objectAtIndex:UndIndex]objectForKey:@"movie"];
         ...
         ZehnerQTKitMovie=[[ZahlenArray objectAtIndex:ZehnerIndex]objectForKey:@"movie"];
	...
	else	//nur Einer
	{
      ...
      // QTMovie holen
		QTMovie* EinerQTKitMovie=[[ZahlenArray objectAtIndex:EinerIndex]objectForKey:@"movie"];
      //NSLog(@"Start: %lld",Start.timeValue);
